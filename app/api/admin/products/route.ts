...
const schema = z
  .object({
    ...
    mainImageUrl: z
      ...
      .trim()
      .url(
        "Enter a complete main image URL starting with http:// or https://.",
      ),
...
function validationError(error: z.ZodError) {
  ...
  return NextResponse.json(
    {
      error: `${labels[field] ?? "Product"}: ${issue?.message ?? "Check this value."}`,
      field,
    },
    { status: 400 },
  );
...
function urlLines(main: string, gallery = "") {
  ...
  for (const url of urls) {
    try {
      ...
      throw new Error(
        `Gallery URLs: “${url}” is not a complete http(s) image URL.`,
      );
    }
  }
  return urls;
...
async function mutate(request: Request, update: boolean) {
  try {
    ...
    if (update && !data.id)
      return NextResponse.json(
        { error: "Product identifier is missing." },
        { status: 400 },
      );
    ...
    if (conflictingId && conflictingId !== data.id) {
      ...
      return NextResponse.json(
        {
          error: `This ${duplicate} is already used by another product. Open that product or enter a unique ${duplicate}.`,
          field: duplicate === "slug" ? "slug" : "sku",
    ...
    const payload = {
      ...
      specifications: {
        ...
        country: data.originCountry || "India",
        material: data.material ?? "",
      },
      personalization_config: {
        enabled: data.isPersonalized,
        fields: personalisationFields,
        instructions: data.personalizationInstructions ?? "",
    ...
    const oldMedia = update
      ? await client
          .from("product_media")
          .select("url,media_type,alt_text,sort_order")
    ...
    const media = urls.map((url, sort_order) => ({
      product_id: saved.data.id,
      url,
      media_type: "image",
      alt_text: data.imageAlt || data.name,
      sort_order,
    }));
    const mediaResult = media.length
      ? await client.from("product_media").insert(media)
      : { error: null };
    if (mediaResult.error) {
      if (update && oldMedia.data?.length)
        await client
          .from("product_media")
          .insert(
            oldMedia.data.map((item) => ({
              ...item,
              product_id: saved.data.id,
            })),
          );
      if (!update)
        await client.from("products").delete().eq("id", saved.data.id);
    ...
    const relationWrites = await Promise.all([
      data.occasionIds.length
        ? client
            ...
            .insert(
              data.occasionIds.map((occasion_id) => ({
                product_id: saved.data.id,
                occasion_id,
              })),
            )
      ...
      data.collectionIds.length
        ? client
            .from("collection_products")
            .insert(
              data.collectionIds.map((collection_id, sort_order) => ({
                product_id: saved.data.id,
                collection_id,
                sort_order,
              })),
    ...
    const relationWriteError = relationWrites.find(
      (result) => result.error,
    )?.error;
    if (relationWriteError) throw relationWriteError;
    const { data: activeOffers, error: offersError } = await client
      .from("coupons")
      .select("id,applicable_products")
      .eq("is_active", true);
    if (offersError) throw offersError;
    const offerIds = new Set(data.offerIds);
    const offerUpdates = (activeOffers ?? []).map((offer) => {
      const products = new Set((offer.applicable_products ?? []).map(String));
      if (offerIds.has(String(offer.id))) products.add(saved.data.id);
      else products.delete(saved.data.id);
      return client
        .from("coupons")
        .update({ applicable_products: [...products] })
        .eq("id", offer.id);
    });
    const offerUpdateResults = await Promise.all(offerUpdates);
    const offerUpdateError = offerUpdateResults.find(
      (result) => result.error,
    )?.error;
    ...
    const hamperResult = data.hamperEligible
      ? await client
          ...
          .upsert(
            { product_id: saved.data.id, is_active: true },
    ...
    await client.from("audit_logs").insert({
      ...
      metadata: {
        sku: saved.data.sku,
        price: saved.data.price,
        status: saved.data.publication_status,
        media: urls.length,
      },
    });
    revalidatePath("/");
    ...
    return NextResponse.json(
      { error: apiErrorMessage(error, "Could not save product.") },
      { status: 400 },
    );
  }
}
