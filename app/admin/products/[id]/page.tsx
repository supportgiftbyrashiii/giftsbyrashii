...
export default async function Page({
  ...
  const [
    ...
    client
      .from("coupons")
      .select("id,code,description,applicable_products")
      .eq("is_active", true)
    ...
    <div className="admin-page">
      <div className="admin-title">
        <div>
          ...
          <p>Every field below is connected to the live catalog.</p>
        </div>
      </div>
      <AdminProductForm
        ...
        selectedOccasions={(occasionLinks.data ?? []).map(
          (item) => item.occasion_id,
        )}
        selectedRecipients={(recipientLinks.data ?? []).map(
          (item) => item.recipient_id,
        )}
        selectedCollections={(collectionLinks.data ?? []).map(
        ...
        offers={(offers.data ?? []).map((offer) => ({
          ...
          selected: (offer.applicable_products ?? []).includes(id),
        }))}
      />
    </div>
  );
